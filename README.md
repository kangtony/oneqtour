### 하나1Q투어(oneQtour ) 
![image](https://user-images.githubusercontent.com/58426170/174231034-b9730c9e-5b37-47c4-be51-6d7b8f87c517.png)

 - 모아모아(적금) 투어하기! 여행할인과 적금금리 환전 혜택을 한번에 하는 상품몰
...
3. 여행상품 삭제 
```
http PUT "http://localhost:8081/tours/{tourId}/delete" (http PUT "http://localhost:8081/tours/1/delete")
```
4. 여행상품 가격 변경 
```
http PUT "http://localhost:8081/tours/{tourId}/update/{tourPrice}" (http PUT "http://localhost:8081/tours/1/update/1000")
```
5. 주문 : 고객이 여행 상품을 1건씩 주문(orderQuantity + 1 됨)
```
...
3. 등록된 고객이 "여행+적금"상품을 구매해본다. 
 - 아까 등록한 여행+적금상품을 구매해본다. 이때는 하나의 Aggregate 이므로 한번에 등록이 되어야 한다.
 - http :8083/cartItems customer="http://localhost:8083/customers/ha@hana.com" items[]="http://localhost:8083/items/1" items[]="http://localhost:8083/items/2"
...
# 이벤트큐 내용 보기 
   ...
   ```
   $kafka_home/bin/kafka-console-consumer.sh --bootstrap-server :9092 --topic tour --from-beginning
   ```
# 이벤트큐 생성자로 직접 이벤트 발생해보기 
 - $kafka_home/bin/kafka-console-producer.sh --broker-list :9092 --topic {topic_name} 
 
 ```
 $kafka_home/bin/kafka-console-producer.sh --broker-list :9092 --topic tour
 ```
...
- kafka producer 를 실행한 후 본과제의 topic인 tour로 생성자 실행 
```
$kafka/bin/kafka-console-producer.sh --broker-list http://localhost:9092 --topic tour
```
- product domain 에서 발행했을 이벤트의 JSON 을 직접 집어넣는다: 직접 데이터 넣어 보면서 테스트 진행 
```
{"eventType":"OrderPlaced","id":1,"productId":1,"qty":3,"productName":"TV"}
```
- store domain의 PolicyHandler에 이벤트가 수신됨을 확인한다. kafka 이벤트 큐( tour topic에 넣은 값이 ) 소비자에 잘 들어 왔는지 확인 
```
$kafka_home/bin/kafka-console-consumer.sh --bootstrap-server :9092 --topic tour --from-beginning
```
# MSA 서비스 확인 법 
...
1. 여행MD가 여행상품을 만들어 등록/수정/삭제한다.
2. 만들어진 여행상품은 1Q Store페이지에 진열/수정/삭제된다.
3. 고객이 1Q Store페이지에서 마음에드는 여행상품을 선택하여 예약한다.
4. 예약과 동시에 결제가 진행된다.
5. 예약이 되면 예약 내역이 업데이트된다.
6. 고객이 예약을 취소할 수 있다.
7. 예약 사항이 취소될 경우 취소 내역이 업데이트된다.
'''
...
# 서브 도메인, 바운디드 컨텍스트 분리
- 팀별 KPI 와 관심사, 상이한 배포주기 등에 따른  Sub-domain 이나 Bounded Context 를 적절히 분리하였고 
  그 분리 기준의 합리성이 충분히 설명되는가?
- 적어도 3개 이상 서비스 분리
...
- [헥사고날 아키텍처] REST Inbound adaptor 이외에 gRPC 등의 Inbound Adaptor 를 추가함에 있어서 
